...
func (blood BloodSpearHandler) ExecuteP2M(player *Player, monster *Monster) {
    ...
    //     TargetY:   monster.Y(),
    // })
    // player.BroadcastPacket(monster.X(), monster.Y(), &packet.GCSkillToObjectOK4{
    //     ObjectID:  player.Id(),
    //     SkillType: SKILL_BLOOD_SPEAR,
    // })
...
func (ignore ParalyzeHandler) ExecuteP2M(player *Player, monster *Monster) {
    ...
    //    ok := &packet.GCSkillToObjectOK1{
    //        SkillType:      SKILL_PARALYZE,
    //        TargetObjectID: monster.Id(),
    //        Duration:       uint16(skillOutput.Duration),
    //    }
    //    player.send <- ok
    // player.Scene.BroadcastPacket(player.X(), player.Y(), &packet.GCSkillToObjectOK3{
    // 	ObjectID:  player.Id(),
    // 	SkillType: SKILL_PARALYZE,
    // 	TargetX:   monster.X(),
    // 	TargetY:   monster.Y(),
    // })
    //   player.Scene.BroadcastPacket(monster.X(), monster.Y(), &packet.GCSkillToObjectOK4{
    //      ObjectID:  monster.Id(),
    //      SkillType: SKILL_PARALYZE,
    //      Duration:  uint16(skillOutput.Duration),
    //   })
