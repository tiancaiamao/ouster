...
func computeVar(beforeValue *string, regx *regexp.Regexp, headsz, tailsz int, withVar func(*string) string) (*string, error) {
	var i int
...
func (c *Config) RawString(section string, option string) (value string, err error) {
	if _, ok := c.section[section]; ok {
		if tValue, ok := c.section[section][option]; ok {
			return tValue.v, nil
		}
	}
...
func (s Section) RawString(option string) (value string, err error) {
	if tValue, ok := s[option]; ok {
		return tValue.v, nil
	}
...
func (c *Config) RawStringDefault(option string) (value string, err error) {
	if tValue, ok := c.section[DEFAULT_SECTION][option]; ok {
		return tValue.v, nil
	}
	return "", OptionError(option)
}
...
func (c *Config) String(section string, option string) (value string, err error) {
	...
	// % variables
	computedVal, err := computeVar(&value, varRegExp, 2, 2, func(varName *string) string {
		lowerVar := *varName
		// search variable in default section as well as current section
		varVal, _ := c.section[DEFAULT_SECTION][lowerVar]
		if _, ok := c.section[section][lowerVar]; ok {
			varVal = c.section[section][lowerVar]
		}
		return varVal.v
	})
	value = *computedVal
	...
	computedVal, _ = computeVar(&value, envVarRegExp, 2, 1, func(varName *string) string {
...
func (s Section) String(option string) (value string, err error) {
	value, err = s.RawString(option)
	if err != nil {
		return "", err
	}
